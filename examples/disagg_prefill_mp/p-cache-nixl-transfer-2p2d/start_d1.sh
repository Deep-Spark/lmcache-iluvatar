...
# Decoder 1: consume P/D KV through vLLM NIXL only (no MP).
...
export VLLM_NIXL_SIDE_CHANNEL_PORT="${NIXL_DECODE1_SIDE_CHANNEL_PORT}"
export NIXL_DECODE_ENGINE_ID="${NIXL_DECODE1_ENGINE_ID}"
export NIXL_KV_PORT="${NIXL_DECODE1_KV_PORT}"
export DECODER_PORT="${DECODER1_PORT}"
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-${DECODER1_GPU}}"

echo "Starting P-cache+NIXL decoder 1 (NIXL only, no MP)"
