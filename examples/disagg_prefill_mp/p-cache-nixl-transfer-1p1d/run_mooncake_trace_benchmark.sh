...
# Mooncake FAST25 trace benchmark for the 1P1D P-cache/NIXL-transfer POC.
#
...
export BENCH_LOG_PREFIX="[mooncake-bench]"
...
TAG_PREFIX="${TAG_PREFIX:-p-cache-nixl-1p1d}"
# When 1, --max-concurrency only gates prefill; decode inflight is unbounded.
DISABLE_DECODE_CONCURRENCY_LIMIT="${DISABLE_DECODE_CONCURRENCY_LIMIT:-0}"
# decode-limited | decode-unlimited | both (runs the A/B pair)
