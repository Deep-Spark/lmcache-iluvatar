...
# Decoder 1 — pair-1 → LMCache Server A (ZMQ 6555). Same-side P→D via local MP.
...
echo "Starting D1 → Server A (mp.port=${LMCACHE_MP_PORT_A}) on IP_A=${IP_A}"
echo "  port=${DECODER1_PORT} CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"
