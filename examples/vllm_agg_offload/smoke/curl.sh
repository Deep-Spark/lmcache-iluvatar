...
# Send one or two completion requests to the running vLLM agg + LMCache MP server.
#
# Quick smoke test:
#   bash curl.sh
#
# Compare COLD vs WARM output (LMCache external prefix hit — layout issues show on WARM):
#   WARM=1 bash curl.sh
#
# Start MP server, then vLLM (layout is fixed at vLLM process start):
#   bash start_lmcache_mp.sh
#   VLLM_KV_CACHE_LAYOUT=NHD CUDA_VISIBLE_DEVICES=0,1 bash start_server.sh
#   WARM=1 bash curl.sh
#
# Override prompt / generation length:
#   PROMPT="Explain KV cache." PROMPT_REPEAT=80 MAX_TOKENS=32 WARM=1 bash curl.sh
...
echo "note: KV layout is chosen when the server started (check vLLM logs for HND/NHD)."
echo ""
...
if [[ "${WARM}" == "1" ]]; then
  send_request "REQ2"
  echo "Compare REQ1 vs REQ2 — garbled REQ2 with hit tokens in server log suggests layout mismatch."
fi
