...
async def lifespan(app: FastAPI):
    ...
    for i, (host, port) in enumerate(decoder_pairs):
        ...
        if incremental_mode:
            # Stride by list length so TP>1 base ports (e.g. 17300,17301)
            # do not overlap across decoders on the same host.
            # num_decoders=1 keeps the previous single-decoder behaviour.
            stride = max(len(global_args.decoder_init_port), 1)
            init_ports = [p + i * stride for p in global_args.decoder_init_port]
            alloc_stride = max(len(global_args.decoder_alloc_port), 1)
            alloc_ports = [
                p + i * alloc_stride for p in global_args.decoder_alloc_port
            ]
        else:
...
async def handle_models():
    """Forward to the first prefiller (readiness probe for bench_wait_for_proxy)."""
    try:
        prefill_client = app.state.prefill_clients[0]
        response = await prefill_client.client.get("/v1/models")
        response.raise_for_status()
