...
class RequestFuncOutput:
    ...
    prefill_ttft: float = 0.0  # PD disagg: proxy-injected prefill token
    itl: List[float] = field(default_factory=list)  # List of inter-token latencies
    ftts: Optional[float] = None
    prompt_len: int = 0
    error: str = ""
    output_len: int = 0
...
def _is_prefill_injected_chunk(data: dict) -> bool:
    return data.get("disagg_source") == "prefill"


def _update_pd_disagg_stream_timing(
    output: RequestFuncOutput,
    *,
    st: float,
    timestamp: float,
    ttft_recorded: bool,
    pd_prefill_seen: bool,
    most_recent_timestamp: float,
    data: dict,
) -> Tuple[bool, bool, float, bool]:
    """Update TTFT/ITL for PD disagg proxy streams.

    Returns (ttft_recorded, pd_prefill_seen, most_recent_timestamp,
    release_decode_semaphore).
    """
    is_prefill = _is_prefill_injected_chunk(data)
    if is_prefill or not pd_prefill_seen:
        if output.prefill_ttft == 0.0:
            output.prefill_ttft = timestamp - st
        return ttft_recorded, True, timestamp, False

    if not ttft_recorded:
        output.ttft = timestamp - st
        output.ftts = timestamp
        return True, pd_prefill_seen, timestamp, True

    output.itl.append(timestamp - most_recent_timestamp)
    return ttft_recorded, pd_prefill_seen, timestamp, False


def _update_standard_stream_timing(
    output: RequestFuncOutput,
    *,
    st: float,
    timestamp: float,
    ...
    """Returns (ttft_recorded, most_recent_timestamp, release_decode_semaphore)."""
    if not ttft_recorded:
        output.ttft = timestamp - st
        output.ftts = timestamp
        return True, timestamp, True

    output.itl.append(timestamp - most_recent_timestamp)
    return ttft_recorded, timestamp, False
                    ...
                    async for chunk_bytes in response.content:
                        ...
                        if ttft == 0.0:
                            ...
                            if (
                                ...
                                and args.disable_decode_concurrency_limit
                            ):
                                semaphore.release()
                                semaphore = None
                    ...
                    async for chunk_bytes in response.content:
                        ...
                        if chunk == "[DONE]":
                            ...
                            if data["choices"][0]["text"]:
                                ...
                                if getattr(args, "pd_disagg_ttft", False):
                                    (
                                        ttft_recorded,
                                        pd_prefill_seen,
                                        most_recent_timestamp,
                                        release_semaphore,
                                    ) = _update_pd_disagg_stream_timing(
                                        output,
                                        st=st,
                                        timestamp=timestamp,
                                        ttft_recorded=ttft_recorded,
                                        pd_prefill_seen=pd_prefill_seen,
                                    ...
                                    (
                                        ...
                                        most_recent_timestamp,
                                        release_semaphore,
                                    ) = _update_standard_stream_timing(
                                        output,
                                        st=st,
                                        timestamp=timestamp,
                                        ttft_recorded=ttft_recorded,
...
def get_tokenizer(
    ...
    return AutoTokenizer.from_pretrained(
        pretrained_model_name_or_path, trust_remote_code=True, fix_mistral_regex=True
    )
...
class BenchmarkMetrics:
    ...
    concurrency: float
    mean_prefill_ttft_ms: Optional[float] = None
    median_prefill_ttft_ms: Optional[float] = None
...
# Match main Mooncake trace replay; warmup uses fewer hash_ids to stay short for 1P1D PD.
MOONCAKE_HI_PER_HASH = 128
MOONCAKE_WARMUP_MAX_HASH_IDS = 2


def build_mooncake_prompt_text(
    hash_ids: List[int],
    *,
    hi_per_hash: int = MOONCAKE_HI_PER_HASH,
    max_hash_ids: Optional[int] = None,
    tail: str = "Tell me a story based on this context.",
) -> str:
    if max_hash_ids is not None:
        hash_ids = hash_ids[:max_hash_ids]
    prompt_text = ""
    hi_token = "hi"
    for hash_id in hash_ids:
        prompt_text += f"{hash_id}" + " ".join([hi_token] * hi_per_hash)
...
async def get_mooncake_request_over_time(
    ...
    for record in input_requests:
        ...
        user_query_base = build_mooncake_prompt_text(hash_ids)
...
def sample_random_requests(
    ...
    if random_sample:
        ...
        offsets = np.random.randint(0, tokenizer.vocab_size, size=num_prompts).tolist()
        input_requests = []
...
async def get_request(
    ...
    if use_trace_timestamps:
        ...
        for request in input_requests_iter:
            ...
            if request_rate == float("inf") and request_interval == None:
                # If the request rate is infinity, then we don't need to wait.
                continue

            if request_interval:
                # Fix the interval time.
...
def calculate_metrics(
    ...
    for i in range(len(outputs)):
        if outputs[i].success:
            ...
            if outputs[i].prefill_ttft > 0.0:
                prefill_ttfts.append(outputs[i].prefill_ttft)
            fttss.append(outputs[i].ftts)
    ...
    dur_prefill_s = max(filter(lambda x: x is not None, fttss)) - ts_start
    dur_decode_s = ts_end - min(filter(lambda x: x is not None, fttss))
    ...
    mean_prefill_ttft_ms = None
    median_prefill_ttft_ms = None
    if prefill_ttfts:
        mean_prefill_ttft_ms = np.mean(prefill_ttfts) * 1000
        median_prefill_ttft_ms = np.median(prefill_ttfts) * 1000
    ...
    metrics = BenchmarkMetrics(
        ...
        input_throughput=total_input / dur_prefill_s,
        output_throughput=sum(output_lens) / dur_decode_s,
        output_throughput_retokenized=sum(retokenized_output_lens) / dur_decode_s,
        ...
        * 1000,  # ttfts is empty if streaming is not supported by backend
        ...
        concurrency=np.sum(ttfts) / dur_prefill_s,
        mean_prefill_ttft_ms=mean_prefill_ttft_ms,
        median_prefill_ttft_ms=median_prefill_ttft_ms,
    )
...
async def benchmark(
    ...
    if args.dataset_name == "mooncake":
        ...
        # Short warmup prompt (same hi/hash ratio as main trace, fewer hash_ids).
        warmup_record = input_requests[0]
        hash_ids = warmup_record.get("hash_ids", [])
        prompt_text = build_mooncake_prompt_text(
            hash_ids,
            max_hash_ids=MOONCAKE_WARMUP_MAX_HASH_IDS,
        )
    ...
    # Run warmup sequentially (respects max_concurrency via semaphore when >1).
    warmup_outputs = []
    ...
    if args.dataset_name == "mooncake":
        ...
        request_generator = get_request(input_requests, request_rate, request_interval)
        request_rate = (1 / request_interval) if request_interval else request_rate

    pbar = None if disable_tqdm else tqdm(total=pbar_total)
    dispatched_requests: List[DatasetRow] = []
    ...
    metrics, output_lens = calculate_metrics(
        input_requests=dispatched_requests,
        outputs=outputs,
        ts_start=benchmark_start_time,
        ts_end=benchmark_end_time,
        tokenizer=tokenizer,
        backend=backend,
    )
    ...
    print(
        "{:<40} {:<10.2f}".format(
            "Benchmark duration (s):", benchmark_end_time - benchmark_start_time
        )
    )
    ...
    if getattr(args, "pd_disagg_ttft", False) and metrics.mean_prefill_ttft_ms is not None:
        print("{s:{c}^{n}}".format(s="Prefill Token (PD proxy)", n=50, c="-"))
        print(
            "{:<40} {:<10.2f}".format(
                "Mean Prefill TTFT (ms):", metrics.mean_prefill_ttft_ms
            )
        )
        print(
            "{:<40} {:<10.2f}".format(
                "Median Prefill TTFT (ms):", metrics.median_prefill_ttft_ms
            )
        )
    ...
    if (
        ...
        result = {
            ...
            "mean_prefill_ttft_ms": metrics.mean_prefill_ttft_ms,
            "median_prefill_ttft_ms": metrics.median_prefill_ttft_ms,
    ...
    result_details = {
        ...
        "prefill_ttfts": [output.prefill_ttft for output in outputs],
...
def run_benchmark(args_: argparse.Namespace):
    ...
    if args.tokenize_prompt:
        assert args.backend in [
            ...
            "vllm",
            "vllm-chat",
        ], "`--tokenize-prompt` only compatible with `--backend sglang` or `--backend vllm` currently"
...
def define_sgl_bench_args(parser: ArgumentParser):
    """
    Defines arguments for the SGLang benchmark (from bench_serving.py).
    """
    parser.add_argument(
        "--backend",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Fixed interval (in seconds) between consecutive requests.If set, this overrides --request-rate, "
        "and scheduling will be deterministic (fixed interval) instead of Poisson. Default is None",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Make '--max-concurrency' only applies to prefill.",
    )
    parser.add_argument(
        "--pd-disagg-ttft",
        action="store_true",
        help="PD disagg: measure TTFT from decode node's first token. "
        "Skips proxy-injected prefill chunks tagged with disagg_source=prefill.",
    )
